...
public class FLapSeluruhPenjualanHarian {

	private JFrame frame;
	static FLapSeluruhPenjualanHarian window;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					window = new FLapSeluruhPenjualanHarian();
	 ...
	 */
	public FLapSeluruhPenjualanHarian() {
		initialize();
	}
	...
	private void initialize() {
		...
		frame.setTitle("Lap. Penjualan Harian");
	...
	private void showReport(){
        String reportName = ".\\Laporan\\LapSeluruhTransaksiHarian.jasper";
        parametersMap.put(
        		"paramTanggal",
	...
	private void TampilTahun(){
		...
		try{
			...
			Statement st;
			ResultSet rs;
			query = "select year(tanggal) as tahun from tb_transaksi_kredit union select year(tanggal) as tahun from tb_transaksi group by tahun";
	...
	private void ExportExcel(){
        ...
        try {
	        ...
	        xlsExporter.setExporterOutput(new SimpleOutputStreamExporterOutput(System.getProperty("user.home") + 
	        		"\\Desktop\\LaporanTransaksiHarian_" +  
	        		"_" +
	        		cmbTanggal.getSelectedItem().toString() + "/" + cmbBulan.getSelectedItem().toString() + "/" + cmbTahun.getSelectedItem().toString() +
	        		".xls"));
        	...
        	JOptionPane.showMessageDialog(null, "Faktur berhasil di-export ke file Ms. Excel\nFile tersimpan di Desktop\n" +
        			"dengan nama file LaporanTransaksiHarian_" + 
	        		"_" +
	        		cmbTanggal.getSelectedItem().toString() + "/" + cmbBulan.getSelectedItem().toString() + "/" + cmbTahun.getSelectedItem().toString() +
	        		".xls");
	...
	private void TampilBulan(){
		...
		try{
			...
			query = "SELECT date_format(tanggal, '%m') as bulan from tb_transaksi_kredit where year(tanggal) = ? group by bulan union select date_format(tanggal, '%m') as bulan from tb_transaksi " +
					"where year(tanggal) = ? group by bulan order by bulan";
