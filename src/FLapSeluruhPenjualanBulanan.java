...
public class FLapSeluruhPenjualanBulanan {

	private JFrame frame;
	static FLapSeluruhPenjualanBulanan window;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					window = new FLapSeluruhPenjualanBulanan();
	 ...
	 */
	public FLapSeluruhPenjualanBulanan() {
		initialize();
	}
	...
	private void initialize() {
		...
		frame.setTitle("Lap. Penjualan Bulanan");
	...
	private void showReport(){
        String reportName = ".\\Laporan\\LapSeluruhTransaksiBulanan.jasper";
        parametersMap.put(
        		"paramBulanTahun",
	...
	private void ExportExcel(){
        ...
        try {
	        ...
	        xlsExporter.setExporterOutput(new SimpleOutputStreamExporterOutput(System.getProperty("user.home") + 
	        		"\\Desktop\\LaporanTransaksiBulanan_" +  
	        		"_" +
	        		cmbBulan.getSelectedItem().toString() + "/" + cmbTahun.getSelectedItem().toString() +
	        		".xls"));
        	...
        	JOptionPane.showMessageDialog(null, "Faktur berhasil di-export ke file Ms. Excel\nFile tersimpan di Desktop\n" +
        			"dengan nama file LaporanTransaksiBulanan_" + 
	        		"_" +
	        		cmbBulan.getSelectedItem().toString() + "/" + cmbTahun.getSelectedItem().toString() +
	        		".xls");
	...
	private void TampilBulan(){
		...
		try{
			...
			query = "SELECT date_format(tanggal, '%m') as bulan from tb_transaksi_kredit "
					+ "where year(tanggal) = ? group by bulan "
					+ "union "
					+ "select date_format(tanggal, '%m') as bulan from tb_transaksi "
					+ "where year(tanggal) = ? group by bulan "
					+ "order by bulan";
